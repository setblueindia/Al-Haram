    ...
    const fetchData = async () => {
      try {
        ...
        if (response.data && response.data.children_data) {
          ...
          response.data.children_data.forEach(category => {
            console.log('name i get::', category.name);
            if (category.children_data) {
              category.children_data.forEach(childCategory => {
                // console.log('child last name:', childCategory.name);
                if (childCategory.children_data) {
                  childCategory.children_data.forEach(childData => {
                    // console.log('data Name:', childData.name);
                  });
                }
              });
            }
    ...
    return (
      <View>
        <View key={category.id}>
          ...
          >
            <TouchableOpacity onPress={() => navigation.navigate('Categorybanner', { categoryId: category.id })}>
              <Text
                style={{
