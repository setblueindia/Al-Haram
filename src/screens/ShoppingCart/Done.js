...
const Done = props => {
  ...
  const senNotiFication = async () => {
    if (result?.data?.status == 'Successful' || offline) {
  ...
  const updateOrderStatus = async () => {
    ...
    fromdata.append(
      'status',
      result?.data?.status == 'Successful' ? 'success' : 'fail',
    );
  ...
  return (
    <View style={styles.mainView}>
        ...
        <View style={styles.lottiView}>
          ...
          <View style={styles.thumIcon}>
            <Image
              style={styles.icon}
              source={
                result?.data?.status == 'Successful' ? doneIcon : ErrorIcon
              }
            />
          </View>
        </View>
      ...
      <View style={styles.textView}>
        {result && (
          <Text style={styles.congrationText}>
            {result?.data?.status == 'Successful' ? Congratulation : oppss}
          </Text>
        )}
      ...
      <View style={styles.btnView}>
        <Button
          onPress={() => {
            navigation.replace(NAVIGATION.MyOrderSscreen);
          }}
          text={lang == NUMBER.num0 ? 'عرض الطلب' : 'View Order'}
        />
        <TouchableOpacity
          onPress={() => {
          ...
          <Text style={styles.continuesShoppingsText}>
            {lang == NUMBER.num0 ? 'متابعة التسوق' : 'Continue Shopping'}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
