...
const Categories = () => {
  const {CategoriesData, navigation, lang, userData, isLoadding} =
    useCategroiesHook();
                ...
                <TouchableOpacity
                  onPress={() => {
                    items?.sub_category?.length > 0
                      ? (setIndex(index), setViewMore(false))
                      : navigation.navigate(NAVIGATION.ProductScreen, {
                          cetegoriesId: items?.category_id,
                        });
                  }}
                  style={[
                    styles.opationView,
                          ...
                          <>
                            <TouchableOpacity
                              ...
                              <FastImage
                                ...
                                style={styles.image}
                                source={
                                  item?.image ? {uri: item?.image} : bannecrSC
                                }
                              />
                            </TouchableOpacity>
