    ...
    <View style={styles.mainView}>
      ...
      <ScrollView style={styles.container}>
        {data.map((items, index) => {
          ...
          const address = `${items?.short_address} ${items?.address1} ${items?.address2}  ${items?.address3}`;
          return (
            <View key={index}>
              <TouchableOpacity
                onPress={() => {
      ...
      <View style={styles.btnView}>
        <TouchableOpacity
          ...
          <Text style={styles.btnText}>
            {lang == NUMBER.num0 ? '+ إضافة عنوان جديد' : '+Add new address'}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      {deletePopp && (
        <Modal visible={deletePopp} animationType="slide" transparent>
          <DeleteBox
            noPress={() => {
              setDetetePopp(false);
            }}
            yesPress={() => {
              setDetetePopp(false), deleteAdress();
            }}
            lang={lang}
          />
        </Modal>
