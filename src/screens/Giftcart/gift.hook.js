...
import {SessionOut} from '../../Hooks/SessionOut';
...
const useGiftHook = props => {
  ...
  const onRecipientPress = () => {
    ...
    if (allFieldsEmpty) {
      ...
      if (!am_giftcard_sender_name) {
        ...
        am_giftcard_recipient_email &&
        !emaileRegxp.test(recipientEmail)
      ) {
  ...
  const getData = async () => {
    ...
    try {
      ...
      const temp = [];
      giftCartInfo?.data?.data?.getGiftCartProductByStore?.media_gallery_entries?.map(
        items => {
  ...
  const onAddTocart = async () => {
    ...
    const recipientDetails1 = recipientDetails.map(card => {
      if (
        card.am_giftcard_message === undefined &&
    ...
    const createAddonRecipientsString = recipients => {
      return recipients
        .map(recipient => {
          ...
          am_giftcard_recipient_email: "${
            recipient.am_giftcard_recipient_email
              ? recipient.am_giftcard_recipient_email
              : ''
          }",
          mobilenumber: "${recipient.mobilenumber}",
          am_giftcard_message: "${
            recipient.am_giftcard_message ? recipient.am_giftcard_message : ''
          }"
    }`;
        })
        .join(',');
    ...
    const addonRecipientsString =
      createAddonRecipientsString(recipientDetails1);
        ...
        `;
          ...
          if (response?.data?.data?.addGiftCartToShoppingCart?.success) {
            ...
            if (!response?.data?.data?.addGiftCartToShoppingCart?.is_login) {
              await SessionOut(
                navigation,
                response?.data?.data?.addGiftCartToShoppingCart?.message,
                dispatch,
              );
            }
            setIsLoadding(false);
          ...
          await SessionOut(navigation, result?.message, dispatch);
          setIsLoadding(false);
        }
      } catch (error) {
      ...
      recipientDetails1.forEach(item => {
        Object.values(item).forEach(value => {
          if (!item?.am_giftcard_recipient_name) {
            ...
            item?.am_giftcard_recipient_email &&
            !emaileRegxp.test(item?.am_giftcard_recipient_email)
          ) {
            SHOWTOTS(langues?.Invalidemailaddress);
            hasUndefinedValue = true;
            setError(true);
            ...
            item?.mobilenumber?.length < 9 ||
            item?.mobilenumber?.length > 10
          ) {
            SHOWTOTS(langues?.Numbercontainsmustbe9digits);
            hasUndefinedValue = true;
          } else {
